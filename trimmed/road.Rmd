...
\
```{r, echo=FALSE, warning=FALSE, message=FALSE, out.width="50%", fig.align="center", fig.width=2}
...
Analysis for ischemic heart disease:
\
```{r, echo=FALSE, warning=FALSE, message=FALSE, results = 'hide'}
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# fixed effects
fixed.ihd=metagen(beta,sebeta,data=ihd,
                     studlab=paste(author,sep=","),
                     comb.fixed=TRUE,comb.random=FALSE,sm="RR")
# random effects
random.ihd=metagen(beta,sebeta,data=ihd,
                     studlab=paste(author,sep=","),
                     comb.fixed=FALSE,comb.random=TRUE,
                     method.tau="PM",prediction=TRUE,hakn=TRUE,sm="RR")
```
...
Perform meta-regression, create object with splines

```{r, echo=FALSE, warning=FALSE, message=FALSE}
# filter dataset
ihdregr= metaregr %>% filter(disease=='Ischemic heart disease')
# add outlier column
ihdregr$outlier=ihd.trim$is_outlier
# filter
ihdregr=ihdregr %>% filter(outlier==0)
# create splines object on the observed noise level exposure
spline_basis=ns(ihdregr$noiselevel, df = 2)
# compute meta-regression
res_spline=rma(yi = beta, sei = sebeta, 
                  mods = spline_basis, 
                  random = ~ 1 | author,
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# create new level of exposure
new_exposure=seq(min(ihdregr$noiselevel),
                 max(ihdregr$noiselevel), length.out = 100)
# fit splines to the new exposure
new_spline_basis=ns(new_exposure, df = 2)
# predict 
pred_spline1=predict(res_spline, newmods = new_spline_basis)
# create df
dfsplines1=data.frame(pred=pred_spline1$pred,
...
\
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(ihdregr,aes(x=noiselevel,y=beta))+
  geom_point(aes(x=noiselevel,y=beta,size=sebeta),alpha=.5,col="#005c99")+
  #geom_label_repel()+
  guides(size='none')+
  labs(x='Noise Exposure',
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
residuals=residuals(res_spline)
ihdregr$residuals=residuals
# extract estimate
estimate=0
se=res_spline$se[1]
se.seq=seq(0, max(ihdregr$sebeta)+0.02, 0.01)
#Compute vectors of the lower-limit and upper limit values for
#the 95% CI region
ll95 = estimate-(1.96*se.seq)
ul95 = estimate+(1.96*se.seq)
...
#You can ignore it.
dfcenter=data.frame(y=estimate,
                    x=seq(0,max(ihdregr$sebeta)+0.02,0.01))
dfCI = data.frame(ll95, ul95, se.seq, estimate)
# Define fill colors for contour
ggplot(data=ihdregr,aes(x=sebeta,y=residuals))+
  geom_point(data=ihdregr,aes(x=sebeta,y=beta,size=sebeta,alpha=0.7))+
  geom_line(aes(x = se.seq, y = ll95), data = dfCI,col='black',alpha=0.8,lty=2) +
  geom_line(aes(x = se.seq, y = ul95), data = dfCI,col='black',alpha=0.8,lty=2) +
  #geom_line(aes(x = se.seq, y = ll99), data = dfCI,col='red') +
  #geom_line(aes(x = se.seq, y = ul99), data = dfCI,col='red') +
...
#Egger's test
egger_test <- lm(beta ~ sebeta, data = ihdregr)
summary(egger_test)
et_pval <- summary(egger_test)$coefficients[2, 4]
print(et_pval)
```
...
stroke=meta %>% 
  ...
  arrange(studyid) %>% 
  mutate(author=ifelse(author=='Cole‑Hunter et al.','Cole - Hunter et al.',author))
# filter other rows
stroke=stroke[-c(2,9,11,12),]
```
\
Compute fixed and random effects models
\
...
```
\
Filter for disease of interest and rows
\
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# filter for the outcome of interest
cvds=meta %>% 
  filter(disease=='Cardiovascular diseases') %>% 
  arrange(studyid)
# filter other rows
cvds=cvds[-c(5),]
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# fixed effects
fixed.cvds=metagen(beta,sebeta,data=cvds,
                     studlab=paste(author,sep=","),
                     comb.fixed=TRUE,comb.random=FALSE,sm="RR")
# random effects
random.cvds=metagen(beta,sebeta,data=cvds,
                     studlab=paste(author,sep=","),
                     comb.fixed=FALSE,comb.random=TRUE,
                     method.tau="PM",prediction=TRUE,hakn=TRUE,sm="RR")
```
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# filter dataset
cvdsregr = metaregr %>% filter(disease=='Cardiovascular diseases')
# add outlier column
cvdsregr$outlier=cvds.trim$is_outlier
# filter
cvdsregr=cvdsregr %>% filter(outlier==0)
# create splines object on the observed noise level exposure
spline_basis=ns(cvdsregr$noiselevel, df = 3)
# compute meta-regression
res_spline=rma(yi = beta, sei = sebeta, 
                  mods = spline_basis, 
                  random = ~ 1 | author,
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
# create new level of exposure
new_exposure=seq(min(cvdsregr$noiselevel),
                 max(cvdsregr$noiselevel), length.out = 100)
# fit splines to the new exposure
new_spline_basis=ns(new_exposure, df = 3)
# predict 
pred_spline1=predict(res_spline, newmods = new_spline_basis)
...
\
```{r, echo=FALSE, warning=FALSE, message=FALSE}
ggplot(cvdsregr,aes(x=noiselevel,y=beta))+
  geom_point(aes(x=noiselevel,y=beta,size=sebeta),alpha=.5,col="#005c99")+
  #geom_label_repel()+
  guides(size='none')+
  labs(x='Noise Exposure',
...
```{r, echo=FALSE, warning=FALSE, message=FALSE}
residuals=residuals(res_spline)
cvdsregr$residuals=residuals
# extract estimate
estimate=0
se=res_spline$se[1]
se.seq=seq(0, max(cvdsregr$sebeta)+0.02, 0.01)
#Compute vectors of the lower-limit and upper limit values for
#the 95% CI region
ll95 = estimate-(1.96*se.seq)
ul95 = estimate+(1.96*se.seq)
...
#You can ignore it.
dfcenter=data.frame(y=estimate,
                    x=seq(0,max(cvdsregr$sebeta)+0.02,0.01))
dfCI = data.frame(ll95, ul95, se.seq, estimate)
# Define fill colors for contour
ggplot(data=cvdsregr,aes(x=sebeta,y=residuals))+
  geom_point(data=cvdsregr,aes(x=sebeta,y=beta,size=sebeta,alpha=0.7))+
  geom_line(aes(x = se.seq, y = ll95), data = dfCI,col='black',alpha=0.8,lty=2) +
  geom_line(aes(x = se.seq, y = ul95), data = dfCI,col='black',alpha=0.8,lty=2) +
  #geom_line(aes(x = se.seq, y = ll99), data = dfCI,col='red') +
  #geom_line(aes(x = se.seq, y = ul99), data = dfCI,col='red') +
...
```{r}
#Egger's test
egger_test=lm(beta ~ sebeta, data = cvdsregr)
summary(egger_test)
et_pval=summary(egger_test)$coefficients[2, 4]
print(et_pval)
```
